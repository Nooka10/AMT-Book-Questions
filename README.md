...
Tomcat n'est pas considéré comme un server d'application à part entière car il n'implémente pas entièrement l'API de JavaEE. 
Mais combiner avec TomEE, comble l'écart entre Tomcat et la spécification JavaEE
...
De nos jour, on a cassé la barrière séparant les développeurs et les operateurs IT (IT operators) et de travailler avec des plus petites équipes. C'est le coeur du principe de l'approche DevOps
...
| -------------------------------- | --------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| La page web sur notre navigateur | Les servlets gérant les endpoint. | Services gérant le shopping cart.<br />Service gérant les transactions bancaires | Liaison avec la base de donnée.<br />Liste de livraison , etc | Base de donnée MySQL ou PostGres. (probablement relationnel) |
...
- Par ligne de commande
  ...
  - (Intellij utilise ca pour deployer depuis son interface graphique)
  - asadmin pour glassfish
- Au démarrage du serveur.
  - On peut définir un dossier contenant les applications à deployer
...
- Lorsque le client fait une requête sur un endpoint demandant ce formulaire, on va construire une page HTML (avec son css et js associés) en ajouter les valeurs par défaut (si il y en a) en parcourant les différents scope (request, session, application).

- Lorsque l'utilisateur aura remplie le formulaire, il appuiera sur le bouton *send* qui ferra une requête poste sur le servlet. Ce dernier devra faire le controle et retournera éventuellement des erreurs ou redirigera la requête vers un autre servlet qui donnera une nouvelle page.
...
- Request
  ...
  - Contient les paramètres de la requête. Ex: L'id de l'applications à afficher
  - Est transféré sur le réseau donc attention au donnée personnel ou confidentiel
  - La request passe dans différents service (sevlet, filter, jsp, etc). C'est un bon moyen de pouvoir faire passer des informations entre ces différents services
- Session
  - Relatif à la session d'un utilisateur
  - Contient les infos de l'utilisateur.
  - Reste du coté serveur
  - On stocker par exemple le status, id, email.
  - Permet d'éviter des requêtes à la base de donnée mais attention peut augmenter la consomation de la ram si on a beaucoup d'utilisateur connecté et qu'on stoque des informations lourdes
  - Le client recoit un cookie mais ne contient pas les données de la session mais uniquement un identifiant.
- Application
  - Partagé entre toute l'application
...
- Ca prends de la mémoire.
- Pose problème lorsque le système commence à être scallé (Sticky Session ?)
